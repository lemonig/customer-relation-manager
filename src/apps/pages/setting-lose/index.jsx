...
function SettingLose() {
  ...
  const handleDel = ({ id }) => {
    Modal.confirm({
      ...
      cancelText: "取消",
      onOk: async () => {
        let { success, message: msg } = await loseDelete({ id });
        if (success) {
  ...
  const handleOk = async () => {
    ...
    if (operateId) {
      values.id = operateId;
      let { success, message: msg } = await loseUpdate(values);
      if (success) {
      ...
      let { success, message: msg } = await loseAdd(values);
      if (success) {
        message.success("提交成功");
        setIsModalOpen(false);
      } else {
  ...
  return (
    <div>
      <PageHeader className="site-page-header" title="丢单类型" />
